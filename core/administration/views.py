...
class AdminIntelligenceView(APIView):
    """View to get high-level system intelligence and trends."""
    ...
    def get(self, request):
        ...
        total_xp = UserProfile.objects.aggregate(total_xp=Sum("xp"))["total_xp"] or 0
        total_completions = UserProgress.objects.filter(status=UserProgress.Status.COMPLETED).count()

        # Metrics
        retention_rate = (active_users / total_users * 100) if total_users > 0 else 0
        engagement_score = (total_completions / total_users * 10) if total_users > 0 else 0
        avg_gems_user = (total_xp / total_users) if total_users > 0 else 0

        # Activity Distribution (last 12 months)
        activity_dist = []
        months_labels = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC']
        
        for i in range(11, -1, -1):
            check_date = now - timedelta(days=i*30)
            month_idx = (check_date.month - 1) % 12
            count = UserProgress.objects.filter(
                status=UserProgress.Status.COMPLETED,
                completed_at__year=check_date.year,
                completed_at__month=check_date.month
            ).count()
            
            # Simulated curve if data is low for demo purposes
        ...
        recent_logs = AdminAuditLog.objects.order_by('-timestamp')[:10]
        events = []
        for log in recent_logs:
            # Simplistic relative time
            ...
            color = "blue"
            if "FAIL" in log.action or "DELETE" in log.action: color = "red"
            elif "SUCCESS" in log.action or "SOLVE" in log.action or "TOGGLE" in log.action: color = "emerald"
            elif "PURCHASE" in log.action: color = "purple"
        ...
        return Response({
            "metrics": {
                ...
                "avg_gems_per_user": round(avg_gems_user, 0),
                "admin_count": staff_count
            },
